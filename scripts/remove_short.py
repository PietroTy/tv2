    ...
    for item in ch.get('videos', []):
        if item.get('type') == 'video':
            ...
            if new_parts:
                ...
                item['dur'] = sum(p.get('dur', 0) for p in new_parts)
                new_videos.append(item)
    ch['videos'] = new_videos
...
print(f"Limpeza concluída! Foram removidos {removed_count} vídeos curtos (< 60s).")
