...
def clean_specific(title):
    # Pica pau numeração: "1 - Andy Panda..." -> "Andy Panda..."
    title = re.sub(r'^\d+\s*-\s*', '', title)
    
    # Chaves: "Chaves - O Ladrão..." -> "O Ladrão..."
    title = re.sub(r'(?i)^Chaves\s*-\s*', '', title)
    
    # Fudêncio: "Fudêncio - Quiprocó..." -> "Quiprocó..."
    title = re.sub(r'(?i)^Fud[êe]ncio(?: e Seus Amigos)?\s*-\s*', '', title)
...
for ch in channels:
    for item in ch.get('videos', []):
        if 'title' in item:
            item['title'] = clean_specific(item['title'])
        if 'parts' in item:
            for part in item['parts']:
                if 'title' in part:
                    part['title'] = clean_specific(part['title'])
...
print("Filtros específicos aplicados!")
