...
for ch in channels:
    if ch.get('name') == 'BOGAROSA GAMES':
        ch['name'] = 'CHAVES'
        ch['label'] = ch['label'].replace('BOGAROSA GAMES', 'CHAVES')
    elif ch.get('name') == 'DIABO NESCESSÁRIO':
        ch['name'] = 'PICA-PAU'
        ch['label'] = ch['label'].replace('DIABO NESCESSÁRIO', 'PICA-PAU')
